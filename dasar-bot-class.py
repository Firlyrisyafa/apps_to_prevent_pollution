...
async def jenis_sampah(ctx):
    num = input("Contoh jenis sampah apa yang ingin kamu ketahui?")
    if num == "organik":
        await ctx.send('-Sisa makanan')
        await ctx.send('-Daun & ranting pohon yang jatuh')
    if num == "unorganik":
        await ctx.send('-Kaleng bekas')  
        await ctx.send('-Botol plastik')      

# tips lingkungan
@bot.command()
async def tips_lingkungan_sederhana(ctx):
    num = random.randint(1,5)
    if num == 1:
        await ctx.send('Matikan peralatan elektronik jika tidak digunakan!')
    if num == 2:
        await ctx.send('Gunakan air seperlunya!')
    if num == 3:
        await ctx.send('Kurangi penggunaan sampah plastik')    
    if num == 4:
        await ctx.send('Lestarikan tanaman disekitar rumah') 
    if num == 5:
        await ctx.send('Gunakan peralatan yang bisa digunakan kembali')    
